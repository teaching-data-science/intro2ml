...
\lecturechapter{2}{Brief History}
...
\vspace{.1cm}
\begin{figure}
\includegraphics[width=6cm]{plots/adaline.png}
\end{figure}
\vspace{1cm}
...
\begin{itemize}
\item Overly optimistic expectations concerning potential of AI/DL.
\item The phrase \enquote{AI} even reached a pseudoscience status.
\item Kernel machines and graphical models both achieved good results on many important tasks.
\item Some fundamental mathematical difficulties in modeling long sequences were identified.
\end{itemize}
\begin{figure}
