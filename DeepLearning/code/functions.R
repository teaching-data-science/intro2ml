...
sd_plot = function(col = terrain_hcl, theta = 40, phi = 40, xlab = "x", ylab = "y", labels = TRUE) {
  ...
  if (labels) {
    ...
    pmat = persp2(x, y, z, theta = theta, phi = phi, ticktype = "detailed",
                  xlab = "", ylab = "", zlab = "", col = col, lwd = 0.5)
  }
...
sd_plot2 = function(col = terrain_hcl, theta = 40, phi = 40, xlab = "x", ylab = "y") {
  if(is.function(col)) col = col(nrow(z) * ncol(z))
  par(mfrow = c(1, 2))
  par(mar = rep(0, 4))
  require("colorspace")
