    ...
    return (
        <>
            ...
            {options.map(({ key, props }, i) => {
                ...
                //console.log(rr_corr)
                //console.log(rr_corr.length)
                //console.log("Selected answers:")
                //console.log(asel)
                //console.log(asel.length)
                var has_equal_length = asel.length === rr_corr.length
                //console.log(has_equal_length)
                var all_corr = true
                if (has_equal_length) {
                  ...
                  all_corr = false
                  //console.log("JUMP TO FALSE")
                }
                //console.log(all_corr)
                console.log(answer)
                return answer === i ? (
                    <div
                        key={key}
                        className={classNames(classes.answer, { [classes.correct]: isCorrect && all_corr })}
                    >
                        <strong
                            className={classNames(classes.answerLabel, {
                                [classes.answerLabelCorrect]: isCorrect && all_corr,
                            })}
                        >
                            {isCorrect && all_corr ? "That's correct! " : 'Incorrect. '}
                        </strong>
                        {props.children}
                    </div>
                ) : null
