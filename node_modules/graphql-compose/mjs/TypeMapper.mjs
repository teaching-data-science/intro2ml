...
export class TypeMapper {
  ...
  isTypeDefinitionString(str) {
    return RegexpOutputTypeDefinition.test(str) || RegexpInputTypeDefinition.test(str) || RegexpEnumTypeDefinition.test(str) || RegexpScalarTypeDefinition.test(str) || RegexpInterfaceTypeDefinition.test(str);
  }
  ...
  convertOutputType(composeType) {
    if (this.schemaComposer.hasInstance(composeType, ObjectTypeComposer)) {
      ...
      const type = this.isTypeDefinitionString(composeType) ? this.createGraphQLType(composeType) : this.getWrapped(composeType);
  ...
  convertInterfaceType(composeType) {
    if (this.schemaComposer.hasInstance(composeType, InterfaceTypeComposer)) {
      ...
      const type = RegexpInterfaceTypeDefinition.test(composeType) ? this.createGraphQLType(composeType) : this.getWrapped(composeType);
